...
func (c *Client) GetNodeInfo(ctx context.Context) (node *NodeInfo, err error) {
	...
	protocol := strings.ToLower(strings.TrimSpace(cm.Protocol))
	if protocol == "hysteria" || protocol == "hy2" {
		protocol = "hysteria2"
	}
	switch protocol {
	case "vmess", "trojan", "hysteria2", "tuic", "anytls", "vless":
		...
		// Protocols that intrinsically run over TLS (Hysteria 2, Tuic, Trojan)
		// must enforce TLS mode even if panels serialize tls: 0 or omit it.
		if (protocol == "hysteria2" || protocol == "tuic" || protocol == "trojan") && node.Security == None {
			node.Security = Tls
		}
	case "shadowsocks":
		node.Type = protocol
	...
	if certDomain == "" {
		if cm.ServerName != "" {
			certDomain = cm.ServerName
		} else if cm.ListenIP != "" && cm.ListenIP != "0.0.0.0" {
			certDomain = cm.ListenIP
		} else {
