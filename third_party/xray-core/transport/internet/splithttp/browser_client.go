...
func (c *BrowserDialerClient) OpenStream(ctx context.Context, url string, sessionId string, body io.Reader, uploadOnly bool) (io.ReadCloser, net.Addr, net.Addr, error) {
	...
	headerCtx, cancelHeader := context.WithTimeout(ctx, responseHeaderTimeout)
	defer cancelHeader()
	conn, err := browser_dialer.DialGetContext(headerCtx, request.URL.String(), request.Header, request.Cookies())
...
func (c *BrowserDialerClient) PostPacket(ctx context.Context, url string, sessionId string, seqStr string, payload buf.MultiBuffer) error {
	...
	headerCtx, cancelHeader := context.WithTimeout(ctx, responseHeaderTimeout)
	defer cancelHeader()
	err = browser_dialer.DialPacketContext(headerCtx, method, request.URL.String(), request.Header, request.Cookies(), bytes)
	if err != nil {
		return err
	}
