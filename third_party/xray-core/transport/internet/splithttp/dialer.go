...
func createHTTPClient(dest net.Destination, streamSettings *internet.MemoryStreamConfig) DialerClient {
	...
	if httpVersion == "3" {
		...
		transport = &http3.Transport{
			...
			MaxResponseHeaderBytes: maxH1ResponseHeaderBytes,
		...
		transport = &http2.Transport{
			...
			MaxHeaderListSize: uint32(maxH1ResponseHeaderBytes),
		}
	} else {
...
func Dial(ctx context.Context, dest net.Destination, streamSettings *internet.MemoryStreamConfig) (stat.Connection, error) {
	...
	lifecycleCtx, cancelLifecycle := context.WithCancel(context.WithoutCancel(ctx))
	...
	if mode == "stream-one" {
		...
		conn.reader, conn.remoteAddr, conn.localAddr, err = httpClient.OpenStream(lifecycleCtx, requestURL.String(), sessionId, reader, false)
		...
		conn.reader, conn.remoteAddr, conn.localAddr, err = httpClient2.OpenStream(lifecycleCtx, requestURL2.String(), sessionId, nil, false)
		if err != nil { // browser dialer only
			_ = conn.Close()
			return nil, err
		}
	...
	if mode == "stream-up" {
		...
		_, _, _, err = httpClient.OpenStream(lifecycleCtx, requestURL.String(), sessionId, reader, true)
		if err != nil { // browser dialer only
			_ = conn.Close()
			return nil, err
		}
	...
	go func() {
		...
		for {
			...
			for doSplit.Store(true); doSplit.Load(); {
				...
				requestCtx := httptrace.WithClientTrace(lifecycleCtx, &httptrace.ClientTrace{
				...
				if dynamicXmuxClient != nil && (dynamicXmuxClient.LeftRequests.Add(-1) <= 0 ||
					...
					dynamicHTTPClient, dynamicXmuxClient = getHTTPClient(lifecycleCtx, dest, streamSettings)
				}
