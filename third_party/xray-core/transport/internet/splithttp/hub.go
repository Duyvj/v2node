...
func (h *requestHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	*/
	...
	if !validSessionID(sessionId) {
		errors.LogInfo(context.Background(), "XHTTP session ID is too long")
		writer.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if sessionId != "" {
		currentSession, err = h.sessions.upsert(sessionId, h.ln.config.GetNormalizedScMaxBufferedPosts())
		if err != nil {
			errors.LogInfoInner(context.Background(), err, "failed to create XHTTP session")
	...
	if isUplinkRequest && sessionId != "" { // stream-up, packet-up
		if seqStr == "" {
			...
			if err != nil {
				...
				if (hasLegacyRefererCompatMarker || obfsPaddingAccepted) && scStreamUpServerSecs.To > 0 {
					go runStreamUpKeepalive(httpSC, h.config, scStreamUpServerSecs)
				}
				select {
		...
		// Claim the combined channel+heap+in-progress slot before walking or
		// allocating payload metadata. Existing IDs use the same admission path.
		packetReservation, err := currentSession.uploadQueue.reserve(seq, nil)
		if err != nil {
			errors.LogInfoInner(context.Background(), err, "failed to reserve XHTTP packet slot")
			writer.WriteHeader(http.StatusConflict)
			return
		}
		transferredReservation := false
		defer func() {
			if !transferredReservation {
				packetReservation.release()
			}
		}()

		// First pass only counts existing request strings. The retained-byte
		// budget is acquired before Join, base64 decode, or body read allocates.
		headerEncodedBytes := 0
		if dataPlacement == PlacementAuto || dataPlacement == PlacementHeader {
			for i := 0; ; i++ {
				...
				headerEncodedBytes += len(chunk)
			}
		}

		cookieEncodedBytes := 0
		if dataPlacement == PlacementAuto || dataPlacement == PlacementCookie {
			for i := 0; ; i++ {
				...
				if c, _ := request.Cookie(cookieName); c != nil {
					cookieEncodedBytes += len(c.Value)
				} else {
					break
				}
			}
		...
		headerBytes := base64.RawURLEncoding.DecodedLen(headerEncodedBytes)
		cookieBytes := base64.RawURLEncoding.DecodedLen(cookieEncodedBytes)
		reservedBytes := int64(headerBytes + cookieBytes)
		readsBody := dataPlacement == PlacementAuto || dataPlacement == PlacementBody
		if readsBody {
			...
			if request.ContentLength > 0 {
				reservedBytes += request.ContentLength
			} else {
				// Unknown/chunked bodies may consume the entire configured packet
				// allowance. Reserving the maximum closes the slow-body loophole.
				reservedBytes = int64(scMaxEachPostBytes)
			}
		}
		if reservedBytes > int64(scMaxEachPostBytes) {
			errors.LogInfo(context.Background(), "Too large upload. scMaxEachPostBytes is set to ", scMaxEachPostBytes, "but request size exceed it. Adjust scMaxEachPostBytes on the server to be at least as large as client.")
			writer.WriteHeader(http.StatusRequestEntityTooLarge)
			return
		}

		byteReservation, err := h.sessions.reservePacketBytes(sessionId, currentSession, packetReservation, reservedBytes)
		if err != nil {
			errors.LogInfoInner(context.Background(), err, "failed to reserve XHTTP packet memory")
			writer.WriteHeader(http.StatusServiceUnavailable)
			return
		}
		var headerPayloadEncoded string
		if dataPlacement == PlacementAuto || dataPlacement == PlacementHeader {
			headerPayloadChunks := make([]string, 0, 1)
			for i := 0; ; i++ {
				chunk := request.Header.Get(fmt.Sprintf("%s-%d", uplinkDataKey, i))
				if chunk == "" {
					break
		...
		var cookiePayloadEncoded string
		if dataPlacement == PlacementAuto || dataPlacement == PlacementCookie {
			cookiePayloadChunks := make([]string, 0, 1)
			for i := 0; ; i++ {
				cookieName := fmt.Sprintf("%s_%d", uplinkDataKey, i)
				if c, _ := request.Cookie(cookieName); c != nil {
		...
		if readsBody {
			...
			trackedBody, trackErr := h.sessions.registerBody(sessionId, currentSession, request.Body, responseController.SetReadDeadline)
			if trackErr != nil {
				errors.LogInfoInner(context.Background(), trackErr, "failed to track XHTTP request body")
				writer.WriteHeader(http.StatusConflict)
				return
			}
			defer func() {
				if trackedBody != nil {
					h.sessions.unregisterBody(currentSession, trackedBody)
				}
			}()
			deadlineSet := trackedBody.armDeadline()
			if request.ContentLength > 0 {
		...
		byteReservation.shrinkTo(int64(len(payload)))
		...
		if sessionId != "" {
			// Once paired, the session is owned by this request until the
			// connection closes and no longer consumes the pending-session budget.
			if err := h.sessions.markConnected(sessionId, currentSession); err != nil {
				errors.LogInfoInner(context.Background(), err, "failed to claim XHTTP downlink")
				writer.WriteHeader(http.StatusConflict)
				return
			}
			defer h.sessions.delete(sessionId, currentSession)
		}
...
func runStreamUpKeepalive(httpSC *httpServerConn, config *Config, interval *RangeConfig) {
	for {
		if httpSC.Done() {
			return
		}
		if _, err := httpSC.Write(bytes.Repeat([]byte{'X'}, int(config.GetNormalizedXPaddingBytes().rand()))); err != nil {
			return
		}
		seconds := interval.rand()
		if seconds <= 0 {
			seconds = 1
		}
...
func (c *httpServerConn) Close() error {
	c.closeOnce.Do(func() {
		_ = c.Instance.Close()
		// A keepalive or downstream write may be in progress. Expire it first,
		// then join through the same mutex used by Write so ServeHTTP cannot
		// return while a goroutine still owns its ResponseWriter.
		if c.ResponseWriter != nil {
			_ = http.NewResponseController(c.ResponseWriter).SetWriteDeadline(time.Now())
		}
		c.Lock()
		defer c.Unlock()
...
func ListenXH(ctx context.Context, address net.Address, port net.Port, streamSettings *internet.MemoryStreamConfig, addConn internet.ConnHandler) (internet.Listener, error) {
	sessions := newSessionStore(defaultMaxPendingSessions, defaultPendingSessionTTL)
	keepSessions := false
	defer func() {
		if !keepSessions {
			_ = sessions.Close()
...
func (ln *Listener) Close() error {
	ln.closeOnce.Do(func() {
		...
		if ln.h3server != nil {
			if err := ln.h3server.Close(); err != nil && !stderrors.Is(err, stdnet.ErrClosed) {
				ln.closeErr = err
			}
			if ln.h3listener != nil {
				if err := ln.h3listener.Close(); ln.closeErr == nil && err != nil && !stderrors.Is(err, stdnet.ErrClosed) {
					ln.closeErr = err
				}
			}
		} else if ln.listener != nil {
			if err := ln.server.Close(); err != nil && !stderrors.Is(err, stdnet.ErrClosed) {
				ln.closeErr = err
			}
			if err := ln.listener.Close(); ln.closeErr == nil && err != nil && !stderrors.Is(err, stdnet.ErrClosed) {
				ln.closeErr = err
			}
		} else {
