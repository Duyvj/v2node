package cnc

import (
	"io"
	"os"
	"sync"
	"sync/atomic"
	"time"

	"github.com/xtls/xray-core/common"
	"github.com/xtls/xray-core/common/buf"
	"github.com/xtls/xray-core/common/net"
	"github.com/xtls/xray-core/common/signal/done"
)

type ConnectionOption func(*Connection)

func ConnectionLocalAddr(a net.Addr) ConnectionOption {
	return func(c *Connection) {
		c.local = a
	}
}

func ConnectionRemoteAddr(a net.Addr) ConnectionOption {
	return func(c *Connection) {
		c.remote = a
	}
}

func ConnectionInput(writer io.Writer) ConnectionOption {
	return func(c *Connection) {
		c.writer = buf.NewWriter(writer)
	}
}

func ConnectionInputMulti(writer buf.Writer) ConnectionOption {
	return func(c *Connection) {
		c.writer = writer
	}
}

func ConnectionOutput(reader io.Reader) ConnectionOption {
	return func(c *Connection) {
		c.reader = &buf.BufferedReader{Reader: buf.NewReader(reader)}
	}
}

func ConnectionOutputMulti(reader buf.Reader) ConnectionOption {
	return func(c *Connection) {
		c.reader = &buf.BufferedReader{Reader: reader}
	}
}

func ConnectionOutputMultiUDP(reader buf.Reader) ConnectionOption {
	return func(c *Connection) {
		c.reader = &buf.BufferedReader{
			Reader:   reader,
			Splitter: buf.SplitFirstBytes,
		}
	}
}

func ConnectionOnClose(n io.Closer) ConnectionOption {
	return func(c *Connection) {
		c.onClose = n
	}
}

func NewConnection(opts ...ConnectionOption) net.Conn {
	c := &Connection{
		done: done.New(),
		local: &net.TCPAddr{
			IP:   []byte{0, 0, 0, 0},
			Port: 0,
		},
		remote: &net.TCPAddr{
			IP:   []byte{0, 0, 0, 0},
			Port: 0,
		},
	}

	for _, opt := range opts {
		opt(c)
	}

	return c
}

type Connection struct {
	reader  *buf.BufferedReader
	writer  buf.Writer
	done    *done.Instance
	onClose io.Closer
	local   net.Addr
	remote  net.Addr

	closeOnce sync.Once
	closeErr  error
	readMu    sync.Mutex

	deadlineMu         sync.Mutex
	readDeadlineTimer  *time.Timer
	writeDeadlineTimer *time.Timer
	readDeadlineGen    uint64
	writeDeadlineGen   uint64
	readExpired        atomic.Bool
	writeExpired       atomic.Bool
}

func (c *Connection) Read(b []byte) (int, error) {
	if c.readExpired.Load() {
		return 0, os.ErrDeadlineExceeded
	}
	if c.done.Done() {
		return 0, io.ErrClosedPipe
	}

	c.readMu.Lock()
	defer c.readMu.Unlock()
	n, err := c.reader.Read(b)
	if n == 0 && c.readExpired.Load() {
		return 0, os.ErrDeadlineExceeded
	}
	return n, err
}

// ReadMultiBuffer implements buf.Reader.
func (c *Connection) ReadMultiBuffer() (buf.MultiBuffer, error) {
	if c.readExpired.Load() {
		return nil, os.ErrDeadlineExceeded
	}
	if c.done.Done() {
		return nil, io.ErrClosedPipe
	}

	c.readMu.Lock()
	defer c.readMu.Unlock()
	mb, err := c.reader.ReadMultiBuffer()
	if mb.IsEmpty() && c.readExpired.Load() {
		return nil, os.ErrDeadlineExceeded
	}
	return mb, err
}

// Write implements net.Conn.Write().
func (c *Connection) Write(b []byte) (int, error) {
	if c.writeExpired.Load() {
		return 0, os.ErrDeadlineExceeded
	}
	if c.done.Done() {
		return 0, io.ErrClosedPipe
	}

	l := len(b)
	mb := make(buf.MultiBuffer, 0, l/buf.Size+1)
	mb = buf.MergeBytes(mb, b)
	err := c.writer.WriteMultiBuffer(mb)
	if err != nil && c.writeExpired.Load() {
		return 0, os.ErrDeadlineExceeded
	}
	return l, err
}

func (c *Connection) WriteMultiBuffer(mb buf.MultiBuffer) error {
	if c.writeExpired.Load() {
		buf.ReleaseMulti(mb)
		return os.ErrDeadlineExceeded
	}
	if c.done.Done() {
		buf.ReleaseMulti(mb)
		return io.ErrClosedPipe
	}

	err := c.writer.WriteMultiBuffer(mb)
	if err != nil && c.writeExpired.Load() {
		return os.ErrDeadlineExceeded
	}
	return err
}

// Close implements net.Conn.Close().
func (c *Connection) Close() error {
	c.closeOnce.Do(func() {
		_ = c.done.Close()
		c.stopDeadlineTimers()
		_ = common.Interrupt(c.reader)
		c.readMu.Lock()
		buf.ReleaseMulti(c.reader.Buffer)
		c.reader.Buffer = nil
		c.readMu.Unlock()
		_ = common.Close(c.writer)
		if c.onClose != nil {
			c.closeErr = c.onClose.Close()
		}
	})
	return c.closeErr
}

// LocalAddr implements net.Conn.LocalAddr().
func (c *Connection) LocalAddr() net.Addr {
	return c.local
}

// RemoteAddr implements net.Conn.RemoteAddr().
func (c *Connection) RemoteAddr() net.Addr {
	return c.remote
}

// SetDeadline implements net.Conn.SetDeadline().
func (c *Connection) SetDeadline(t time.Time) error {
	c.setReadDeadline(t, true)
	c.setWriteDeadline(t, true)
	return nil
}

// SetReadDeadline implements net.Conn.SetReadDeadline().
func (c *Connection) SetReadDeadline(t time.Time) error {
	c.setReadDeadline(t, false)
	return nil
}

// SetWriteDeadline implements net.Conn.SetWriteDeadline().
func (c *Connection) SetWriteDeadline(t time.Time) error {
	c.setWriteDeadline(t, false)
	return nil
}

func (c *Connection) setReadDeadline(t time.Time, expireBoth bool) {
	// BufferedReader/Writer do not expose independent recoverable deadlines.
	// Expiry is therefore terminal: mark the expired direction first so its
	// blocked operation deterministically reports os.ErrDeadlineExceeded, then
	// Close interrupts both directions without creating a goroutine per call.
	c.deadlineMu.Lock()
	if c.done.Done() {
		c.deadlineMu.Unlock()
		return
	}
	c.readDeadlineGen++
	gen := c.readDeadlineGen
	if c.readDeadlineTimer != nil {
		c.readDeadlineTimer.Stop()
		c.readDeadlineTimer = nil
	}
	c.readExpired.Store(false)
	if t.IsZero() {
		c.deadlineMu.Unlock()
		return
	}

	d := time.Until(t)
	if d <= 0 {
		c.readExpired.Store(true)
		if expireBoth {
			c.writeExpired.Store(true)
		}
		c.deadlineMu.Unlock()
		_ = c.Close()
		return
	}
	c.readDeadlineTimer = time.AfterFunc(d, func() {
		c.deadlineMu.Lock()
		if gen != c.readDeadlineGen {
			c.deadlineMu.Unlock()
			return
		}
		c.readExpired.Store(true)
		if expireBoth {
			c.writeExpired.Store(true)
		}
		c.deadlineMu.Unlock()
		_ = c.Close()
	})
	c.deadlineMu.Unlock()
}

func (c *Connection) setWriteDeadline(t time.Time, expireBoth bool) {
	c.deadlineMu.Lock()
	if c.done.Done() {
		c.deadlineMu.Unlock()
		return
	}
	c.writeDeadlineGen++
	gen := c.writeDeadlineGen
	if c.writeDeadlineTimer != nil {
		c.writeDeadlineTimer.Stop()
		c.writeDeadlineTimer = nil
	}
	c.writeExpired.Store(false)
	if t.IsZero() {
		c.deadlineMu.Unlock()
		return
	}

	d := time.Until(t)
	if d <= 0 {
		c.writeExpired.Store(true)
		if expireBoth {
			c.readExpired.Store(true)
		}
		c.deadlineMu.Unlock()
		_ = c.Close()
		return
	}
	c.writeDeadlineTimer = time.AfterFunc(d, func() {
		c.deadlineMu.Lock()
		if gen != c.writeDeadlineGen {
			c.deadlineMu.Unlock()
			return
		}
		c.writeExpired.Store(true)
		if expireBoth {
			c.readExpired.Store(true)
		}
		c.deadlineMu.Unlock()
		_ = c.Close()
	})
	c.deadlineMu.Unlock()
}

func (c *Connection) stopDeadlineTimers() {
	c.deadlineMu.Lock()
	defer c.deadlineMu.Unlock()
	c.readDeadlineGen++
	c.writeDeadlineGen++
	if c.readDeadlineTimer != nil {
		c.readDeadlineTimer.Stop()
		c.readDeadlineTimer = nil
	}
	if c.writeDeadlineTimer != nil {
		c.writeDeadlineTimer.Stop()
		c.writeDeadlineTimer = nil
	}
}
