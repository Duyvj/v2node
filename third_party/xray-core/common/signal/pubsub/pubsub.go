...
func (s *Service) Cleanup() error {
	...
	for name, subs := range s.subs {
		...
		// Allocate from the live count rather than the peak slice length. A
		// topic with one survivor after a large churn burst must not retain a
		// peak-sized backing array indefinitely.
		newSub := make([]*Subscriber, 0, live)
		for _, sub := range subs {
			if !sub.IsClosed() {
