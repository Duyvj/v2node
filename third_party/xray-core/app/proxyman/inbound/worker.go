...
func (c *udpConn) Close() error {
	c.closeOnce.Do(func() {
		...
		// Interrupt releases queued packet buffers immediately. Merely closing the
		// writer lets a stopped UDP handler retain and drain them later.
		common.Must(common.Interrupt(c.reader))
		common.Must(common.Interrupt(c.writer))
		if cancel != nil {
			cancel()
		}
	})
...
// Each UDP flow owns a pipe and a proxy goroutine. A spoofed-source packet
// flood must not be able to create an unbounded number of either.
const maxUDPWorkerConnections = 1024
...
func (w *udpWorker) getConnection(id connID) (*udpConn, bool) {
	...
	if len(w.activeConn) >= maxUDPWorkerConnections {
		return nil, false
	}
...
func (w *udpWorker) callback(b *buf.Buffer, source net.Destination, originalDest net.Destination) {
	...
	// WriteMultiBuffer takes ownership of b on both success and error. The pipe
	// also releases it when the bounded buffer overflows or has been closed.
...
func (w *udpWorker) removeConn(id connID, conn *udpConn) {
	...
	if w.activeConn[id] == conn {
		delete(w.activeConn, id)
		if len(w.activeConn) == 0 && !w.closed {
