...
const (
	attackDefenseShardCount   = 32
	maxDefenseRecordsPerShard = 2048
	maxWhitelistPerShard      = 1024
)
...
type AttackDefense struct {
	shards    [attackDefenseShardCount]*defenseShardDB // 32个分片，降低锁竞争
	whitelist [attackDefenseShardCount]*whitelistShardDB
...
type defenseShardDB struct {
	...
	order   []defenseRecordSlot
	next    int
}

// defenseRecordSlot provides bounded FIFO eviction without retaining an
// unbounded auxiliary queue. The record pointer prevents an old slot from
// deleting a newer record for the same address.
type defenseRecordSlot struct {
	addr   string
...
type whitelistSlot struct {
	ip    string
	entry *whitelistEntry
}

type whitelistShardDB struct {
	mu      sync.RWMutex
...
func NewAttackDefense(config *DefenseConfig) *AttackDefense {
	...
	// 初始化32个分片
	for i := 0; i < attackDefenseShardCount; i++ {
		d.shards[i] = &defenseShardDB{
			records: make(map[string]*failureRecord),
		}
		d.whitelist[i] = &whitelistShardDB{
...
func (d *AttackDefense) IsWhitelisted(addr string) bool {
	...
	shard := d.getWhitelistShard(ip)
	shard.mu.RLock()
	entry, exists := shard.entries[ip]
	if !exists {
		shard.mu.RUnlock()
		return false
	}
...
func (d *AttackDefense) RecordSuccess(addr string) {
	...
	// 加入当前 validator 的有界白名单（使用纯IP，1小时过期）。
	expireTime := time.Now().Add(1 * time.Hour).UnixNano()
	d.addWhitelist(ip, expireTime)
}
...
// addRecord inserts a new record while keeping the shard strictly bounded.
// The caller must hold s.mu for writing.
func (s *defenseShardDB) addRecord(addr string, record *failureRecord) {
	slot := defenseRecordSlot{addr: addr, record: record}
	if len(s.order) < maxDefenseRecordsPerShard {
		s.order = append(s.order, slot)
	} else {
		evicted := s.order[s.next]
		if s.records[evicted.addr] == evicted.record {
			delete(s.records, evicted.addr)
		}
		s.order[s.next] = slot
		s.next = (s.next + 1) % maxDefenseRecordsPerShard
	}
	s.records[addr] = record
}

// compactOrder releases addresses and records already removed from the map.
// The caller must hold s.mu for writing.
func (s *defenseShardDB) compactOrder() {
	if len(s.records) == len(s.order) {
		return
	}
	compacted := make([]defenseRecordSlot, 0, len(s.records))
	for i := 0; i < len(s.order); i++ {
		...
		slot := s.order[index]
		if s.records[slot.addr] == slot.record {
...
func (d *AttackDefense) getShard(addr string) *defenseShardDB {
	...
	return d.shards[hash%attackDefenseShardCount]
}
...
// Close stops background cleanup and releases all per-instance records. It is
// safe to call more than once.
func (d *AttackDefense) Close() error {
	if d == nil {
		return nil
	}
	d.closeOnce.Do(func() {
		...
		for i := 0; i < attackDefenseShardCount; i++ {
			...
			shard.next = 0
			shard.mu.Unlock()
			whitelist := d.whitelist[i]
			whitelist.mu.Lock()
			whitelist.entries = nil
			whitelist.order = nil
			whitelist.next = 0
			whitelist.mu.Unlock()
		}
...
// cleanupWhitelist clears expired entries and rebuilds FIFO storage so stale
// source-IP strings are released instead of retaining historical peaks.
...
func (d *AttackDefense) getWhitelistShard(ip string) *whitelistShardDB {
	hash := uint32(0)
	for i := 0; i < len(ip); i++ {
		hash = hash*31 + uint32(ip[i])
	}
	return d.whitelist[hash%attackDefenseShardCount]
}

func (d *AttackDefense) addWhitelist(ip string, expires int64) {
	if ip == "" || d.closed.Load() {
		return
	}
	shard := d.getWhitelistShard(ip)
	...
	if entry := shard.entries[ip]; entry != nil {
		entry.expires = expires
		return
	}
	entry := &whitelistEntry{expires: expires}
	slot := whitelistSlot{ip: ip, entry: entry}
	if len(shard.order) < maxWhitelistPerShard {
		shard.order = append(shard.order, slot)
	} else {
		evicted := shard.order[shard.next]
		if shard.entries[evicted.ip] == evicted.entry {
			delete(shard.entries, evicted.ip)
		}
		shard.order[shard.next] = slot
		shard.next = (shard.next + 1) % maxWhitelistPerShard
	}
	shard.entries[ip] = entry
}

func (s *whitelistShardDB) compactOrder() {
	if len(s.entries) == len(s.order) {
		return
	}
	compacted := make([]whitelistSlot, 0, len(s.entries))
	for i := 0; i < len(s.order); i++ {
		index := i
		if len(s.order) == maxWhitelistPerShard {
			index = (s.next + i) % len(s.order)
		}
		slot := s.order[index]
		if s.entries[slot.ip] == slot.entry {
			compacted = append(compacted, slot)
		}
	}
	s.order = compacted
...
func (d *AttackDefense) cleanup() {
	...
	for i := 0; i < attackDefenseShardCount; i++ {
		...
		for {
			...
			if len(shard.records) > maxDefenseRecordsPerShard {
				// 强制删除最旧的记录
				count := 0
			...
			if recordCount <= maxDefenseRecordsPerShard && len(toDelete) < batchSize {
				break
			}
