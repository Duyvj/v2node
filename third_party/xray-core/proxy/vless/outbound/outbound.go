...
type Handler struct {
	...
	reverseStartAccess sync.Mutex
	reverseStartTimer  *time.Timer
	closed             bool
...
func New(ctx context.Context, config *Config) (*Handler, error) {
	...
	if a.Reverse != nil {
		...
		handler.scheduleReverseStart(2 * time.Second)
	}
...
func (h *Handler) scheduleReverseStart(delay time.Duration) {
	h.reverseStartAccess.Lock()
	defer h.reverseStartAccess.Unlock()

	if h.closed || h.reverse == nil {
		return
	}
	if h.reverseStartTimer != nil {
		h.reverseStartTimer.Stop()
	}
	h.reverseStartTimer = time.AfterFunc(delay, h.startReverse)
}

func (h *Handler) startReverse() {
	h.reverseStartAccess.Lock()
	defer h.reverseStartAccess.Unlock()

	h.reverseStartTimer = nil
	if h.closed || h.reverse == nil {
		return
	}
	if err := h.reverse.Start(); err != nil {
		errors.LogWarningInner(h.reverse.ctx, err, "failed to start VLESS reverse monitor")
	}
}
...
func (h *Handler) startPreconnections(dialer internet.Dialer, destination net.Destination) {
	h.reverseStartAccess.Lock()
	defer h.reverseStartAccess.Unlock()
	if h.closed {
		return
	...
	h.preCancel = cancel
	for range h.testpre {
		workerCtx := xctx.ContextWithID(ctx, session.NewID())
		go runPreconnector(workerCtx, dialer, destination, preConns)
	}
}

func runPreconnector(ctx context.Context, dialer internet.Dialer, destination net.Destination, ready chan<- *ConnExpire) {
	for {
		...
		select {
		case ready <- &ConnExpire{Conn: conn, Expire: time.Now().Add(2 * time.Minute)}:
		case <-ctx.Done():
			_ = conn.Close()
			return
		}
...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	if h.testpre > 0 && h.reverse == nil {
		h.initpre.Do(func() {
			h.startPreconnections(dialer, rec.Destination)
		})
		h.reverseStartAccess.Lock()
		preConns := h.preConns
		preDone := h.preDone
...
func (r *Reverse) monitor() error {
	...
	if r.closed {
		return errors.New("VLESS reverse monitor is closed")
	}

	activeWorkers := make([]*reverse.BridgeWorker, 0, len(r.workers))
	for _, w := range r.workers {
		if w.IsActive() {
			activeWorkers = append(activeWorkers, w)
		} else {
			closeReverseWorker(w)
		}
	}
...
func (r *Reverse) Start() error {
	...
	r.access.Lock()
	closed := r.closed
	monitorTask := r.monitorTask
	r.access.Unlock()
	if closed {
		return errors.New("VLESS reverse monitor is closed")
	}
	if monitorTask == nil {
		return errors.New("VLESS reverse monitor task is missing")
	}
	return monitorTask.Start()
}
...
func (r *Reverse) Close() error {
	...
	var errs []error
	errs = append(errs, common.CloseIfExists(monitorTask))
	for _, worker := range workers {
		errs = append(errs, closeReverseWorker(worker))
	}
	return errors.Combine(errs...)
}

func closeReverseWorker(worker *reverse.BridgeWorker) error {
	if worker == nil {
		return nil
	}
	if worker.Timer != nil {
		worker.Timer.SetTimeout(0)
	}
	return common.CloseIfExists(worker.Worker)
}
