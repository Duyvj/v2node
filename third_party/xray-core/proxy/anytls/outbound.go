...
var errClientClosed = errors.New("anytls: client closed")
...
func NewClient(ctx context.Context, config *ClientConfig) (*Client, error) {
	...
	go func() {
		defer client.cleanupWG.Done()
		client.cleanupIdleSessions()
	}()
	return client, nil
}

// Close stops background maintenance and closes all sessions owned by the
// client. Outbound Handler.Close calls common.Close on its proxy, so this is
// also invoked when the Xray core is shut down or replaced during a reload.
...
func (c *Client) markSessionIdle(sess *session) {
	if sess == nil || sess.isClosed() || c.closed.Load() {
		return
	}
