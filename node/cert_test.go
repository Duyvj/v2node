...
func TestCertificateGenerationRejectsPanelControlledPathsOutsideZNode(t *testing.T) {
	...
	if !pathWithinCertificateRoot("/etc/znode/nodes/node.cer", []string{"/etc/v2node", "/etc/znode"}) {
		t.Fatal("valid ZNode certificate path was rejected")
	}
	if !pathWithinCertificateRoot("/etc/v2node/hysteria21.cer", []string{"/etc/v2node", "/etc/znode"}) {
		t.Fatal("valid v2node certificate path was rejected")
	}
