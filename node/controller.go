...
// Prepare validates the panel snapshots before a reload closes live listeners.
func (c *Controller) Prepare(ctx context.Context) error {
	...
	if c.info == nil || c.info.Common == nil {
		return fmt.Errorf("empty node configuration")
	}
	c.tag = c.info.Tag
...
func (c *Controller) Close() error {
	...
	if c.renewCertPeriodic != nil {
		if err := c.renewCertPeriodic.Close(); err != nil {
			return err
		}
	}
	if c.limiter != nil {
	...
	c.apiClient.Close()
	if c.server == nil || !c.inboundActive {
		return nil
	}
