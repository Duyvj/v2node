...
func hardenWireGuardOutbound(outbound *conf.OutboundDetourConfig) error {
	if outbound == nil || outbound.Protocol != "wireguard" {
		return nil
	}
	if outbound.Settings == nil || len(*outbound.Settings) == 0 {
		return nil
	...
	if err != nil {
		return fmt.Errorf("encode wireguard settings: %w", err)
	}
	outbound.Settings = (*json.RawMessage)(&encoded)
	return nil
}
