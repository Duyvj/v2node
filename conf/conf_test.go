...
func TestOriginalV2NodeConfigLoadsSuccessfully(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.json")
	data := []byte(`{
    "Log": {
        "Level": "warning",
	...
	c := New()
	if err := c.LoadFromPath(path); err != nil {
		t.Fatalf("original v2node config failed to load: %v", err)
	}
	if c.Type != "v2board" {
		t.Fatalf("expected default type v2board, got: %s", c.Type)
	}
	if len(c.NodeConfigs) != 1 || c.NodeConfigs[0].NodeID != 81 {
		t.Fatalf("expected node ID 81, got: %+v", c.NodeConfigs)
	}
	if !*c.NodeConfigs[0].DisableSniffing {
		t.Fatal("expected DisableSniffing default true")
	}
}
