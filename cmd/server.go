...
func serverHandle(_ *cobra.Command, _ []string) {
	...
	if err := applyRuntimeConfig(c.Runtime); err != nil {
		log.WithField("err", err).Error("Apply runtime configuration failed")
		return
	}
...
func reload(config string, nodes **node.Node, v2core **core.V2Core) error {
	...
	if err := applyRuntimeConfig(newConf.Runtime); err != nil {
		return err
	}
	...
	newNodes, err := node.New(newConf.NodeConfigs, newConf.Runtime)
	if err != nil {
		return err
	}
